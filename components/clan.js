...
export default function Clan(props) {
    ...
    // Fetch data from API
    const fetcher = (url) =>
        fetch(url, { headers: { "X-API-Key": API_KEY } }).then((res) =>
            res.json()
        );
    ...
    if (error || !data || data.Response.results.length === 0) {
        return (
            <div
                className={`flex flex-col border border-${clanName.toLowerCase()} bg-gray-900`}>
                <header
        ...
        return (
            <div
                className={`flex flex-col bg-gray-900 w-full border border-${clanName.toLowerCase()}`}>
                <header
                    className={`text-white p-4 bg-no-repeat bg-right bg-${clanName.toLowerCase()} ${clanName.toLowerCase()}`}>
                    <h2 className="text-4xl font-thin py-1 my-6">{clanName}</h2>
                    <div className="flex flex-row">
